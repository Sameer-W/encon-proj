...
module.exports.renderAddEntry = async (req, res) => {
    ...
    try {
        ...
        //     await project.save()
        // }
        return res.render("employees/employee-task", {projects, employee, maxDate, minDate, workDescriptions: ["test1", "jbdwdhwkdwodwoqd", "test2", "test3", "test4", "test5", "test6", "test7", "test8", "test9"]})
    }
    catch {
...
module.exports.addEntry = async (req, res) => {
    ...
    let {date, project, hours, workDescription} = req.body;
    ...
    try {
        const newEntry = new Entry({date: finalDate, employeeId, projectId, projectName, workDescription, hours});
